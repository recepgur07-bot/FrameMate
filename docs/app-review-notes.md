...
- Screen Recording: used for full screen and window recording, and for system audio capture where available. If Screen Recording is granted while FrameMate is already running, macOS may briefly need the app to return to the foreground (or, rarely, a relaunch) before capture becomes available — this is a macOS TCC caching behavior, not app-specific.
- Accessibility (AX): requested only if the user turns on the optional "Keyboard Shortcuts" overlay in Settings, so the app can install a system-wide recording shortcut. This is off by default and not required to use the app. VoiceOver support itself uses only the standard macOS accessibility APIs and needs no extra permission.
...
Access model: no account or purchase is required to start recording. New users get 3 days of unlimited local recording (device-only, no StoreKit trial involved), then a permanent free quota of 7 minutes of recording per calendar month that renews automatically — this is tracked entirely on-device and never touches StoreKit. In-app purchases (FrameMate Pro) are yearly auto-renewable subscription and lifetime one-time purchase ($49.99) options for unlimited recording, reachable from the paywall (shown once the free quota runs out, or on demand) and from Settings. The paywall includes Restore Purchases, the privacy policy link, and the Terms of Use (Apple standard EULA) link. The yearly subscription has no App Store introductory offer/free trial — the free experience is handled locally as described above. TestFlight and App Store review builds require a valid entitlement, an active local trial/free quota, or a purchase for recording access — there is no hidden tester unlock.
